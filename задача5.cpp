...
int calculate_nonlinear_explicit_write_file(FILE* file, FILE* file_table, int Ntau, int Mh)
{
    ...
    for (int i = 0; i < Mh + 1; i++)
    {
        printf("V[%d]= %.4f   Sol= %.4f\n", i, Vup[i], nonlinear_solution(1., -1. + i * h));
...
int main(void){
   ...
   // Расчет линейной задачи с явной схемой
    ...
    calculate_nonlinear_exlicit_write_file(File1, File2, 10, 200);
    calculate_nonlinear_explicit_write_file(File1, File2, 100, 200);
   calculate_nonlinear_exlicit_write_file(File1, File2, 1000, 200);
    calculate_nonlinear_exlicit_write_file(File1, File2, 10, 2000);
     calculate_nonlinear_exlicit_write_file(File1, File2, 100, 2000);
   ...
   // Таблица 1
    ...
    // Таблица 2
   calculate_nonlinear_implicit_write_file(File7, File8, 10, 2000, pow(10, -9), 0.155);
   calculate_nonlinear_implicit_write_file(File7, File8, 10, 20000, pow(10, -9), 0.155);
   calculate_nonlinear_implicit_write_file(File7, File8, 5, 2000, pow(10, -9), 0.155);
   calculate_nonlinear_implicit_write_file(File7, File8, 10, 10000, pow(10, -9), 0.155);
   calculate_nonlinear_implicit_write_file(File7, File8, 5, 20000, pow(10, -9), 0.155);
   calculate_nonlinear_implicit_write_file(File7, File8, 5, 1000, pow(10, -9), 0.155);
